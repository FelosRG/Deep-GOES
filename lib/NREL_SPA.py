"""
Descripción:
Calcula la posición del sol usando el algoritmo SPA del NREL.

Última modificación:
29 de Marzo 2022

Autores/Fuentes:
Adaptación del código original de Tony Lorenzo (@alorenzo175), Univ. of Arizona, 2015
https://pvlib-python.readthedocs.io/en/stable/_modules/pvlib/spa.html
por Adrián Ramírez, Facultad de Ciencias, UNAM.
"""
...
def celestial_sun_coordinates(t):
    """
    Calcula la posción del sol en términos de
    las ascención recta y la declinación.
    Función basada en el código original de pvlib.
    """
    ...
    jd = julian_day_dt(year,month,day,hour,minute,second,microsecond)
    ...
    return alpha,delta,lon_o


def sub_orbital_point_sun(asr,dec,lon_o):
    """
    Con las coordenadas astronómicas del sol, se calcula
    su punto sub-orbital.
    """
    
    # Obtenemos la longitud origen de las coordenadas astronómicas
    offset_base = lon_o

    # Calculamos la coordenada de la longitud
    theta_lon = offset_base #frac_dia*360 + offset_base
    
    # Ajustamos al formato abitual de la longitud
    #if theta_lon > 180: theta_lon = -(360 - theta_lon)
    
    theta_lat = round(dec,3)
    theta_lon = round(theta_lon,3)
    
    return theta_lat , theta_lon

def posicion_suborbital_del_sol(t):
    """
    Dada una hora UTC (datetime.datetime.utcnow) obtiene
    las coordenadas sub orbitales del sol
    """
    arc,dec,lon_o = celestial_sun_coordinates(t)
    theta_lat , theta_lon = sub_orbital_point_sun(arc,dec,lon_o)
    return theta_lat , theta_lon
...
if __name__ == "__main__":
    ...
    print(solar_position_numpy(unixtime=unix_time,lat=lat,lon=lon,elev=elv,pressure=pressure,temp=temp))
